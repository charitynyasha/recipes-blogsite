...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      tagsArray = tagsString ? tagsString.split(',').map(tag => tag.trim()).filter(tag => tag) : []
    }
    ...
    if (coverImageFile && coverImageFile.size > 0) {
      ...
      try {
        ...
        coverImageUrl = `data:${coverImageFile.type};base64,${buffer.toString('base64')}`
...
const db = client.db('test')  // ← Add your database name here
    ...
    return NextResponse.json(
      { error: 'Failed to fetch blog posts: ' + (error instanceof Error ? error.message : 'Unknown error') },
      { status: 500 }
    )
  }
}